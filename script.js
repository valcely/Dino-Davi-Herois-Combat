/* ============================================================
   DAVI DINO HERÓIS — script.js  v3.0
   Sem remoção de funcionalidades. Apenas adições e ajustes.
============================================================ */
...
// ============================================================
//  PERSONAGENS
//  ► Mamãe e Davi são os mais fortes por definição
//  ► Heróis: +1000 pts extras de força
//  ► HP calibrado para ~60s de batalha
// ============================================================
var CHARS = {
  /* ── HERÓIS ── */
  daviHeroi: {
    ...
    speed:7.0, str:3.2, sp:'Raio', sp2:'Chute',   // MAIS FORTE do jogo
    desc:'O herói principal!',
    bodyColor:'#00695c', markColor:'#ffd740', letter:'D', kawaii:true
  },
  mamae: {
    name:'MAMÃE DINO', role:'hero', emoji:'🌸',
    color:'#f48fb1', accent:'#fce4ec', lc:'#fce4ec',
    speed:5.5, str:3.0, sp:'Coração', sp2:'Abraço', // 2ª MAIS FORTE
    desc:'Força de mãe!',
    bodyColor:'#c2185b', markColor:'#fce4ec', letter:'M', kawaii:true, pink:true
  },
  aranha: {
    name:'HOMEM-ARANHA', role:'hero', emoji:'🕷️',
    color:'#e53935', accent:'#1565c0', lc:'#ef9a9a',
    speed:7.0, str:2.2, sp:'Teia', sp2:'Chutar',
    desc:'Amigão da vizinhança!',
    bodyColor:'#b71c1c', markColor:'#1565c0', letter:'A', kawaii:true, spiderMan:true
  },
  papai: {
    name:'PAPAI DINO', role:'hero', emoji:'💪',
    color:'#42a5f5', accent:'#ff5252', lc:'#bbdefb',
    speed:4.5, str:2.0, sp:'Super', sp2:'Soco',
    desc:'Guardião da família!',
    bodyColor:'#1565c0', markColor:'#ff5252', letter:'P', kawaii:true
  },
  vovo: {
    name:'VOVÔ DINO', role:'hero', emoji:'🦴',
    color:'#ff8a65', accent:'#ffd740', lc:'#ffab91',
    speed:3.8, str:2.4, sp:'Rugido', sp2:'Bengala',
    desc:'Força de vovô!',
    bodyColor:'#bf360c', markColor:'#ffd740', letter:'V', kawaii:true
  },
  vova: {
    name:'VOVÓ DINO', role:'hero', emoji:'💜',
    color:'#ce93d8', accent:'#fff9c4', lc:'#e1bee7',
    speed:4.6, str:1.9, sp:'Carinho', sp2:'Cabeçada',
    desc:'Amorosa e forte!',
    bodyColor:'#6a1b9a', markColor:'#fff9c4', letter:'G', kawaii:true, bow:true
  },
  tiaTina: {
    name:'TIA TINA', role:'hero', emoji:'✨',
    color:'#ba68c8', accent:'#f3e5f5', lc:'#e1bee7',
    speed:5.8, str:1.8, sp:'Magia', sp2:'Empurrão',
    desc:'Mágica e alegre!',
    bodyColor:'#6a1b9a', markColor:'#f3e5f5', letter:'T', kawaii:true, bow:true
  },
  tiaGio: {
    name:'TIA GIO', role:'hero', emoji:'❄️',
    color:'#80cbc4', accent:'#e0f2f1', lc:'#b2dfdb',
    speed:5.2, str:1.8, sp:'Gelo', sp2:'Chutar',
    desc:'Fresca e rápida!',
    bodyColor:'#00695c', markColor:'#e0f2f1', letter:'G', kawaii:true
  },
  /* ── VILÕES — resistência calibrada para ~60s ── */
  dinoBravo: {
    name:'DINO BRAVO', role:'villain', emoji:'🔥',
    color:'#f44336', accent:'#ff6f00', lc:'#ffcdd2',
    speed:5.8, str:1.4, sp:'Fogo', sp2:'Mordida',
    desc:'Vilão feroz!',
    bodyColor:'#b71c1c', markColor:'#ff6f00', letter:'B', ai:'aggressive'
  },
  dinoSombra: {
    name:'DINO SOMBRA', role:'villain', emoji:'🌑',
    color:'#9c27b0', accent:'#e040fb', lc:'#e1bee7',
    speed:5.2, str:1.2, sp:'Sombra', sp2:'Garras',
    desc:'Das trevas!',
    bodyColor:'#4a148c', markColor:'#e040fb', letter:'S', ai:'ranged'
  },
  dinoRex: {
    name:'DINO-REX', role:'villain', emoji:'👑',
    color:'#ff6f00', accent:'#ffd740', lc:'#ffe0b2',
    speed:4.0, str:1.8, sp:'Tremor', sp2:'Pisão',
    desc:'O Rei dos Vilões!',
    bodyColor:'#e65100', markColor:'#ffd740', letter:'R', ai:'aggressive', king:true
  },
  dinoVeneno: {
    name:'DINO VENENO', role:'villain', emoji:'☠️',
    color:'#76ff03', accent:'#1b5e20', lc:'#ccff90',
    speed:6.5, str:1.1, sp:'Veneno', sp2:'Cuspir',
    desc:'Venenoso e ágil!',
    bodyColor:'#33691e', markColor:'#76ff03', letter:'V', ai:'ranged', poison:true
  },
  dinoGelo: {
    name:'DINO GELO', role:'villain', emoji:'🧊',
    color:'#80deea', accent:'#006064', lc:'#e0f7fa',
    speed:4.5, str:1.3, sp:'Congelar', sp2:'Rastejar',
    desc:'Congela tudo!',
    bodyColor:'#006064', markColor:'#80deea', letter:'G', ai:'normal', icy:true
  },
  dinoGigante: {
    name:'DINO GIGANTE', role:'villain', emoji:'🦴',
    color:'#8d6e63', accent:'#ffd740', lc:'#d7ccc8',
    speed:2.6, str:2.2, sp:'Terremoto', sp2:'Esmagar',
    desc:'Grandão e brutal!',
    bodyColor:'#4e342e', markColor:'#ffd740', letter:'X', ai:'aggressive', giant:true
  }
};

var HERO_IDS    = ['daviHeroi','mamae','aranha','papai','vovo','vova','tiaTina','tiaGio'];
var VILLAIN_IDS = ['dinoBravo','dinoSombra','dinoRex','dinoVeneno','dinoGelo','dinoGigante'];
var ALL_IDS     = HERO_IDS.concat(VILLAIN_IDS);

// ============================================================
//  HP e DANO calibrados para ~60 segundos de batalha
//  Heróis têm bônus extra de +1000 pts (escala interna 0.001)
// ============================================================
var HP_MAX       = 100;
var HIT_DMG_BASE = 0.55;   // dano base por soco (% de HP por golpe)
var SP_DMG_BASE  = 1.8;    // dano especial (% de HP)
var HERO_BONUS   = 1.0;    // +1000 pts = +1.0 na escala interna
var SP_CD        = 4500;
...
function drawDino(ctx, id, x, y, facing, action, frame, isHit, scale) {
  ...
  ctx.translate(0,bob);
  var atk=action==='attack', atk2=action==='attack2', ko=action==='ko';
  var walk=action==='walk'||action==='jump';
  var leg=(walk||atk||atk2)?Math.sin(frame*0.24)*14:0;
  var tail=walk?Math.sin(frame*0.19)*5:0;
  var bc=ch.bodyColor||ch.color, mc=ch.markColor||ch.accent;
  if (ch.spiderMan) _drawSpiderMan(ctx,ch,atk,atk2,ko,leg,tail,frame,id);
  else              _drawKawaiiDino(ctx,ch,bc,mc,atk,atk2,ko,leg,tail,frame,id);
  if (isHit) {
    ctx.globalAlpha=0.42; ctx.fillStyle='rgba(255,60,60,0.5)';
    ctx.fillRect(-50,-102,100,102); ctx.globalAlpha=1;
  }
  ctx.restore();
}
...
function _drawSpiderMan(ctx, ch, atk, atk2, ko, leg, tail, frame, id) {
  ...
  for (var ls=-1;ls<=1;ls+=2) {
    ctx.save(); ctx.translate(ls<0?-10:10,8); ctx.rotate(ls*leg*0.022);
    ctx.fillStyle=bc; ctx.beginPath(); ctx.ellipse(0,6,7,10,ls*0.15,0,Math.PI*2); ctx.fill();
  ...
  ctx.fillStyle=bodyG; ctx.beginPath(); ctx.ellipse(0,-14,24,22,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=mc; ctx.font='bold 15px Arial Black,sans-serif';
  ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('⚡',0,-14);
  ...
  if (atk2) { ctx.strokeStyle=mc; ctx.lineWidth=7; ctx.beginPath(); ctx.moveTo(-10,8); ctx.lineTo(-28,-10); ctx.stroke(); }
  ...
  ctx.fillStyle=hg; ctx.beginPath(); ctx.ellipse(20,-54,17,15,0,0,Math.PI*2); ctx.fill();
  ...
  for (var wi=0;wi<4;wi++) { var wa=wi*Math.PI/4-Math.PI/2; ctx.beginPath(); ctx.moveTo(20,-54); ctx.lineTo(20+Math.cos(wa)*17,-54+Math.sin(wa)*15); ctx.stroke(); }
  ctx.globalAlpha=1;
...
function _drawKawaiiDino(ctx, ch, bc, mc, atk, atk2, ko, leg, tail, frame, id) {
  ...
  for (var ls=-1;ls<=1;ls+=2) {
    ctx.save(); ctx.translate(ls<0?-11:11,9); ctx.rotate(ls*leg*0.024);
    ctx.fillStyle=bc; ctx.beginPath(); ctx.ellipse(0,6,7,10,ls*0.18,0,Math.PI*2); ctx.fill();
    ctx.strokeStyle=bc; ctx.lineWidth=7; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(0,14); ctx.lineTo(ls*3,26); ctx.lineTo(ls*7,32); ctx.stroke();
  ...
  ctx.fillStyle=bodyG; ctx.beginPath(); ctx.ellipse(0,-14,25,23,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='rgba(255,255,255,0.14)';
  ctx.beginPath(); ctx.ellipse(-1,-11,15,14,0.1,0,Math.PI*2); ctx.fill();
  ctx.fillStyle=mc;
  for (var di=0;di<4;di++) { ctx.beginPath(); ctx.ellipse(-8+di*5.5,-31-di*2+3,3.5,5,0,0,Math.PI*2); ctx.fill(); }
  ctx.fillStyle=mc; ctx.globalAlpha=0.9;
  ctx.font='bold 13px Fredoka One,Arial Black,sans-serif';
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText(ch.letter||'?',0,-14); ctx.globalAlpha=1;
  ctx.fillStyle='rgba(0,0,0,0.12)';
  for (var si=0;si<5;si++) { ctx.beginPath(); ctx.arc(-10+si*5.5,-22+Math.sin(si)*2,2.5,0,Math.PI*2); ctx.fill(); }
  ...
  if (atk2) { ctx.strokeStyle=mc; ctx.lineWidth=7; ctx.lineCap='round'; ctx.beginPath(); ctx.moveTo(-10,8); ctx.lineTo(-26,-8); ctx.stroke(); }
  ctx.strokeStyle=bc; ctx.lineWidth=14; ctx.lineCap='round';
  ...
  ctx.fillStyle=hg; ctx.beginPath(); ctx.ellipse(20,-53,17,15,0,0,Math.PI*2); ctx.fill();
  ...
  if (atk||atk2) { ctx.beginPath(); ctx.arc(20,-45,5,-0.2,Math.PI+0.2); ctx.stroke(); }
  else           { ctx.beginPath(); ctx.arc(20,-46,4,0.1,Math.PI-0.1);  ctx.stroke(); }
  if (atk||atk2) {
    ctx.fillStyle='#f5f5e8';
    for (var ti=0;ti<3;ti++) { ctx.beginPath(); ctx.rect(13+ti*4.5,-45,3.5,4); ctx.fill(); }
  }
  if (ko) _drawKO(ctx,frame);
}
...
function _drawCharAccessory(ctx, ch, id, frame, bc, mc) {
  if (id==='vova'||id==='tiaTina') {
    ...
    ctx.fillStyle='#f48fb1'; ctx.font='12px Arial';
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('♥',17,-70); ctx.fillText('♥',26,-72); ctx.fillText('♥',20,-75);
  } else if (id==='tiaGio') {
    ctx.fillStyle='rgba(224,242,241,0.9)'; ctx.font='10px Arial';
    ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('❄',20,-70);
  } else if (id==='vovo') {
    ctx.fillStyle=bc; ctx.beginPath(); ctx.ellipse(20,-68,15,5,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=hexShift(bc,-20); ctx.fillRect(12,-78,17,12);
    ctx.fillStyle=mc; ctx.fillRect(11,-70,19,3);
  } else if (id==='papai') {
    ctx.save(); ctx.strokeStyle=mc; ctx.lineWidth=3; ctx.globalAlpha=0.7;
    ...
    ctx.fillStyle='#e53935'; ctx.beginPath(); ctx.arc(20,-80,3.5,0,Math.PI*2); ctx.fill();
    ...
    ctx.fillStyle='#d7ccc8'; ctx.fillText('🦴',20,-70);
  }
}
...
function drawFightBG(ctx, W, H, GY) {
  ...
  for (var si=0;si<18;si++) { ctx.beginPath(); ctx.moveTo(si*52,GY+64); ctx.lineTo(si*52+26,GY+64); ctx.stroke(); }
  for (var ti=0;ti<9;ti++) {
    var tx=[25,115,210,310,420,520,620,720,830][ti], th=100+(ti%3)*38;
    ctx.fillStyle=ti%2===0?'#0a1e0d':'#0c2612'; ctx.fillRect(tx-4,GY+48-th,8,th);
...
function drawProjs(ctx, projs) {
  for (var i=0;i<projs.length;i++) {
    ...
    if (pr.type==='web') {
      ...
      for (var wi=0;wi<6;wi++) { var wa=wi*Math.PI/3; ctx.beginPath(); ctx.moveTo(pr.x,pr.y); ctx.lineTo(pr.x+Math.cos(wa)*15,pr.y+Math.sin(wa)*15); ctx.stroke(); }
    } else if (pr.type==='heart') {
      ctx.font='22px Arial'; ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.globalAlpha=0.92; ctx.fillText('💗',pr.x,pr.y);
      ...
      ctx.fillStyle='rgba(255,82,82,0.85)'; starPath5(ctx,pr.x,pr.y,10); ctx.fill();
    } else if (pr.type==='quake'||pr.type==='shockwave') {
      ctx.strokeStyle=pr.type==='quake'?'rgba(141,110,99,0.8)':'rgba(0,230,118,0.8)'; ctx.lineWidth=4;
    ...
    } else if (pr.type==='ice'||pr.type==='ice_hero') {
      ctx.fillStyle=pr.type==='ice'?'rgba(128,222,234,0.88)':'rgba(128,203,196,0.88)';
      ...
      ctx.fillStyle='rgba(255,255,255,0.6)'; ctx.font='14px Arial'; ctx.textAlign='center'; ctx.fillText('✦',pr.x,pr.y+6);
...
function drawPops(ctx, pops) {
  for (var i=0;i<pops.length;i++) {
    ...
    ctx.fillStyle=h.c; ctx.textAlign='center'; ctx.textBaseline='alphabetic'; ctx.fillText(h.w,0,0);
    ctx.restore();
  }
}
// ============================================================
...
(function() {
  ...
  for (var i=0;i<160;i++) stars.push({x:Math.random(),y:Math.random(),r:Math.random()*1.6+0.3,sp:Math.random()*3+1,t:Math.random()*Math.PI*2});
  function animStars(){
...
function updateClock(){
  ...
  var el=document.getElementById('clockEl'); if(el) el.textContent=h+':'+m+':'+s;
...
function startHomeAnim(){
  ...
  var pc=document.getElementById('homePreview'); if(!pc) return;
  var ctx=pc.getContext('2d');
  var ids=['daviHeroi','aranha','mamae','papai'], xs=[42,130,230,330];
  function loop(){
    ctx.clearRect(0,0,400,110);
    var fr=Math.round(Date.now()/80)%200;
    for(var i=0;i<4;i++){ctx.save();ctx.translate(xs[i],90);drawDino(ctx,ids[i],0,0,1,'idle',fr+i*12,false);ctx.restore();}
...
function buildCharGrid(){
  ...
  ALL_IDS.forEach(function(id){
    ...
    (function(ctx2,iid){setTimeout(function(){ctx2.clearRect(0,0,80,74);ctx2.save();ctx2.translate(28,62);drawDino(ctx2,iid,0,0,1,'idle',20,false);ctx2.restore();},0);})(cv.getContext('2d'),id);
    circle.appendChild(cv); card.appendChild(circle);
    var nm=document.createElement('div'); nm.className='card-name'; nm.textContent=ch.name; nm.style.color=ch.lc; card.appendChild(nm);
...
function pickChar(id){
  ...
  if(selMode==='1p'){
    ...
    if(pickTurn%2===0) selP1=id; else selP2=id; pickTurn++;
  }
  buildCharGrid(); updateVsPreview();
}
...
function addPop(x,y,big,custom){
  ...
  if(big){var h2=HQ_LIST[Math.floor(Math.random()*HQ_LIST.length)];pops.push({x:x-44,y:y+18,hq:h2,big:false,rot:-14,life:44});}
}

function calcDmg(attacker, isSpecial){
  var ch=CHARS[attacker.cId];
  var base=isSpecial?SP_DMG_BASE:HIT_DMG_BASE;
  var dmg=base*ch.str;
  if(ch.role==='hero') dmg+=HERO_BONUS*(isSpecial?0.4:0.12);
  return dmg;
}
...
function trigAtk(atk,def,hk){
  ...
  if(Math.abs(atk.x-def.x)<CHARS[atk.cId].speed*16+32&&!def.stun&&def.hp>0)
    applyHit(atk,def,calcDmg(atk,false),false,hk);
}

function trigAtk2(atk,def,hk){
  if(atk.atking||atk.atk2ing||atk.stun||atk.hp<=0) return;
  atk.atk2ing=true; atk.atk2T=ATK_DUR*0.8; atk.action='attack2';
  if(Math.abs(atk.x-def.x)<CHARS[atk.cId].speed*14+28&&!def.stun&&def.hp>0)
    applyHit(atk,def,calcDmg(atk,false)*0.85,false,hk);
}

function trigSp(user,opp,hk){
  if(user.spCD>0||user.stun||user.hp<=0) return;
  var ch=CHARS[user.cId];
  user.spCD=SP_CD; user.action='special'; playSound('special');
  var id=user.cId,ptype='lightning';
  ...
  var dmgSp=calcDmg(user,true);
  if(ptype==='quake'||ptype==='roar')
    gs.projs.push({type:ptype,x:user.x,y:GY,r:8,maxR:ptype==='roar'?140:130,prog:0,owner:user,hk:hk,hit:false,dmgSp:dmgSp});
  else
    gs.projs.push({type:ptype,x:user.x+user.facing*26,y:user.y-38,vx:user.facing*PROJ_SPD*(ptype==='fire'?1.6:1.2),spin:0,owner:user,hk:hk,hit:false,dmgSp:dmgSp});
...
// ── Controles touch com espaçamento aumentado ──
function buildCtrl(){
...
function addCtrl(cId,ch,onAtk,onAtk2,onSp,onJump,onJoy){
  ...
  function mkBtn(cls,bg,bc,html,fn){
    ...
    b.style.background=bg; b.style.border='2px solid '+bc;
    b.style.boxShadow='0 0 10px '+bc+'55'; b.innerHTML=html;
  ...
  // 4 botões originais + espaçamento aumentado via gap no CSS
...
function startFight(){
  stopFight(); showScreen('fightScreen');
  ...
  var usedH=(hud.offsetHeight||44)+(spb.offsetHeight||16)+(ctrl.offsetHeight||110);
...
function fightLoop(){
  ...
  function physics(p){
    ...
    if(p.hp>0&&!p.atking&&!p.atk2ing){var opp=p===p1?p2:p1;p.facing=opp.x>p.x?1:-1;}
  }
  physics(p1); physics(p2);
  var kill=[];
  gs.projs.forEach(function(pr){
    if(pr.type==='quake'||pr.type==='roar'){
      pr.r+=4; pr.prog=pr.r/pr.maxR;
      var tgt=pr.owner===p1?p2:p1;
      if(!pr.hit&&tgt.hp>0&&!tgt.stun){
        var hit2=pr.type==='roar'?(Math.abs(pr.x-tgt.x)<pr.r&&Math.abs(pr.y-(tgt.y-30))<pr.r*0.5):Math.abs(pr.x-tgt.x)<pr.r;
        if(hit2){pr.hit=true;applyHit(pr.owner,tgt,pr.dmgSp,true,pr.hk);}
      }
      ...
      if(!pr.hit&&Math.abs(pr.x-tgt.x)<50&&Math.abs(pr.y-(tgt.y-36))<62){
        pr.hit=true; applyHit(pr.owner,tgt,pr.dmgSp,true,pr.hk); kill.push(pr);
      }
...
function goResult(wk,cId){
  ...
  showScreen('resultScreen'); playSound('win');
  ...
  resInterv=setInterval(function(){
    rf++; rctx.clearRect(0,0,140,120);
    rctx.save(); rctx.translate(46,100);
    drawDino(rctx,cId,0,0,1,'idle',rf,false); rctx.restore();
  },50);
...
function buildMemGrid(){
  ...
  memState.cards.forEach(function(card,i){
    ...
    (function(ctx2,iid,s2){setTimeout(function(){ctx2.clearRect(0,0,s2,s2);ctx2.save();ctx2.translate(s2*0.36,s2*0.86);drawDino(ctx2,iid,0,0,1,'idle',20,false,s2/88);ctx2.restore();},0);})(cv.getContext('2d'),card.id,sz);
...
function memFlip(idx){
  ...
  if(memState.flipped.length===2){
    ...
    if(memState.cards[a].pair===memState.cards[b].pair){
      memState.matched.push(a,b); playSound('memMatch');
      setTimeout(function(){
        ...
        var pf=memState.matched.length/2;
        document.getElementById('memPairs').textContent=pf;
        if(pf===memState.pairs) memWin();
      },400);
    } else {
      playSound('memWrong');
      setTimeout(function(){
...
function memWin(){
  if(memState.timer) clearInterval(memState.timer);
  playSound('win');
  document.getElementById('memWinScore').textContent='⭐ '+memState.moves+' jogadas em '+memState.time+' segundos! ⭐';
  document.getElementById('memWin').classList.remove('hidden');
}

// ============================================================
//  PINTURA DE DINOS — v3 com barra fixa e imagens de colorir
// ============================================================
var paintCtx=null, paintTool='brush', paintColor='#ff5252', paintDino='daviHeroi';
var paintDrawing=false, paintLastX=0, paintLastY=0, paintSoundThrottle=0;
var paintCurrentImage=null;   // imagem externa carregada (ou null = dino canvas)
var paintImageLoading=false;

// 24 cores vibrantes
var PAINT_COLORS=[
  '#ff1744','#ff5252','#ff4081','#f48fb1',
  '#e040fb','#ce93d8','#651fff','#7986cb',
  '#29b6f6','#4dd0e1','#00e676','#69f0ae',
  '#ffd740','#ffab40','#ff6d00','#ff8a65',
  '#8d6e63','#fff9c4','#fff','#e0e0e0',
  '#9e9e9e','#616161','#212121','#000'
];

// Temas com imagens de contorno externas (proxy CORS-friendly via AllOrigins / direto CDN público)
// Usamos apenas domínios que permitem hotlink: openclipart, publicdomainvectors, wikimedia, etc.
var PAINT_THEMES=[
  {
    id:'dinos', label:'🦕 DINOS',
    images:[
      {label:'T-Rex',    url:'https://upload.wikimedia.org/wikipedia/commons/thumb/8/83/T_rex_coloring_Nobu_Tamura.svg/800px-T_rex_coloring_Nobu_Tamura.svg.png'},
      {label:'Bronto',   url:'https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Brontosaurus_coloring_Nobu_Tamura.svg/800px-Brontosaurus_coloring_Nobu_Tamura.svg.png'},
      {label:'Triceratops', url:'https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Triceratops_coloring_Nobu_Tamura.svg/800px-Triceratops_coloring_Nobu_Tamura.svg.png'}
    ]
  },
  {
    id:'herois', label:'🦸 HERÓIS',
    images:[
      {label:'Aranha',    url:'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a0/SpiderManColoring.svg/600px-SpiderManColoring.svg.png'},
      {label:'Hulk',      url:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Hulk_coloring.svg/600px-Hulk_coloring.svg.png'},
      {label:'Homem Ferro',url:'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5e/Iron_Man_coloring.svg/600px-Iron_Man_coloring.svg.png'}
    ]
  },
  {
    id:'patrulha', label:'🐾 PATRULHA',
    images:[
      {label:'Ryder',  url:'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d7/Paw_patrol_coloring.svg/600px-Paw_patrol_coloring.svg.png'},
      {label:'Chase',  url:'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Chase_paw_coloring.svg/600px-Chase_paw_coloring.svg.png'},
      {label:'Marshall',url:'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6e/Marshall_coloring.svg/600px-Marshall_coloring.svg.png'}
    ]
  },
  {
    id:'paisagens', label:'🌳 PAISAGENS',
    images:[
      {label:'Floresta', url:'https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Forest_coloring_page.svg/800px-Forest_coloring_page.svg.png'},
      {label:'Cidade',   url:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4c/City_coloring_page.svg/800px-City_coloring_page.svg.png'},
      {label:'Praia',    url:'https://upload.wikimedia.org/wikipedia/commons/thumb/b/b7/Beach_coloring_page.svg/800px-Beach_coloring_page.svg.png'}
    ]
  },
  {
    id:'davino', label:'🎨 DAVI DINO',
    images:[]   // usa os dinos desenhados no canvas (sem imagem externa)
  }
];

var paintThemeIdx=4;  // começa em DAVI DINO
...
function initPaint(){
  ...
  PAINT_COLORS.forEach(function(c){
    ...
    if(c==='#fff'||c==='#fff9c4'||c==='#e0e0e0') btn.style.boxShadow='inset 0 0 0 1px #aaa';
  ...
  // Botões de tema
  var themeBtns=document.getElementById('paintThemeBtns'); themeBtns.innerHTML='';
  PAINT_THEMES.forEach(function(theme,ti){
    var btn=document.createElement('button');
    btn.className='paint-theme-btn'+(ti===paintThemeIdx?' active':'');
    btn.textContent=theme.label;
    btn.onclick=function(){
      paintThemeIdx=ti;
      document.querySelectorAll('.paint-theme-btn').forEach(function(b){b.classList.remove('active');});
      btn.classList.add('active');
      buildPaintSubTheme();
    };
  ...
  // Canvas de pintura — 100% largura, retangular
  var cv=document.getElementById('paintCanvas');
  paintCtx=cv.getContext('2d');
  var topbar=document.getElementById('paintTopbar');
  var tbH=topbar?topbar.offsetHeight:130;
  cv.width=window.innerWidth;
  cv.height=Math.max(window.innerHeight-tbH-10,280);

  buildPaintSubTheme();
  bindPaintCanvas(cv);
}

function buildPaintSubTheme(){
  var theme=PAINT_THEMES[paintThemeIdx];
  // Remove seletor de dino anterior se existir
  var old=document.getElementById('paintDinoSel');
  if(old) old.innerHTML='';

  if(theme.id==='davino'){
    // Mostra seletor de personagem, sem texto de nome (só emoji)
    var dsel=document.getElementById('paintDinoSel');
    if(!dsel){
      dsel=document.createElement('div'); dsel.id='paintDinoSel';
      dsel.className='paint-dinosel';
      var area=document.querySelector('.paint-canvas-area');
      if(area) area.insertBefore(dsel,area.firstChild);
    }
    dsel.innerHTML='';
    ...
    ALL_IDS.forEach(function(id){
      ...
      btn.textContent=CHARS[id].emoji;   // só emoji, sem nome
      btn.title=CHARS[id].name;
      btn.onclick=function(){
        paintDino=id; paintCurrentImage=null;
    ...
    paintCurrentImage=null;
    redrawPaintBg();
  } else {
    // Esconde seletor de dino
    var dsel2=document.getElementById('paintDinoSel');
    if(dsel2) dsel2.style.display='none';

    // Mostra botões de imagens do tema
    // Injeta abaixo do topbar (reutiliza mesma div ou cria)
    var imgRow=document.getElementById('paintImgRow');
    if(!imgRow){
      imgRow=document.createElement('div'); imgRow.id='paintImgRow';
      imgRow.className='paint-img-row';
      var area=document.querySelector('.paint-canvas-area');
      if(area) area.insertBefore(imgRow,area.firstChild);
    }
    imgRow.innerHTML='';
    imgRow.style.display='flex';

    theme.images.forEach(function(img,ii){
      var btn=document.createElement('button');
      btn.className='paint-img-btn';
      btn.textContent=img.label;
      btn.onclick=function(){
        document.querySelectorAll('.paint-img-btn').forEach(function(b){b.classList.remove('active');});
        btn.classList.add('active');
        loadPaintImage(img.url);
      };
      imgRow.appendChild(btn);
    });

    // Carrega primeira imagem automaticamente
    if(theme.images.length>0){
      imgRow.children[0] && imgRow.children[0].classList.add('active');
      loadPaintImage(theme.images[0].url);
    }
  }
}

function loadPaintImage(url){
  paintImageLoading=true;
  var cv=document.getElementById('paintCanvas');
  // Feedback visual de carregamento
  paintCtx.fillStyle='#f0f0f0'; paintCtx.fillRect(0,0,cv.width,cv.height);
  paintCtx.fillStyle='#aaa'; paintCtx.font='18px Nunito,sans-serif';
  paintCtx.textAlign='center'; paintCtx.fillText('Carregando imagem... 🦕',cv.width/2,cv.height/2);

  var img=new Image();
  img.crossOrigin='anonymous';
  img.onload=function(){
    paintImageLoading=false;
    paintCurrentImage=img;
    redrawPaintBg();
  };
  img.onerror=function(){
    paintImageLoading=false;
    paintCurrentImage=null;
    // Fallback: desenha dino canvas com aviso
    redrawPaintBg();
    paintCtx.fillStyle='rgba(255,80,80,0.18)';
    paintCtx.fillRect(0,0,cv.width,cv.height);
    paintCtx.fillStyle='#c00'; paintCtx.font='15px Nunito,sans-serif';
    paintCtx.textAlign='center';
    paintCtx.fillText('⚠️ Imagem indisponível. Pinte o dino! 🦕',cv.width/2,30);
  };
  img.src=url;
}
...
function redrawPaintBg(){
  ...
  if(paintCurrentImage){
    // Desenha imagem de colorir centralizada, mantendo proporção
    var iw=paintCurrentImage.naturalWidth, ih=paintCurrentImage.naturalHeight;
    var scale=Math.min(cv.width/iw, cv.height/ih)*0.92;
    var dw=iw*scale, dh=ih*scale;
    var dx=(cv.width-dw)/2, dy=(cv.height-dh)/2;
    paintCtx.globalAlpha=0.92;
    paintCtx.drawImage(paintCurrentImage,dx,dy,dw,dh);
    ...
    // Texto guia (sem nome completo — só emoji)
    paintCtx.fillStyle='rgba(180,180,200,0.55)';
    paintCtx.font='14px Nunito,sans-serif'; paintCtx.textAlign='center';
    paintCtx.fillText('🎨 Pinte! '+CHARS[paintDino].emoji,cv.width/2,22);
  }
}

function bindPaintCanvas(cv){
  cv.onmousedown=function(e){paintDrawing=true;var r=cv.getBoundingClientRect();paintLastX=e.clientX-r.left;paintLastY=e.clientY-r.top;paintDo(paintLastX,paintLastY,paintLastX,paintLastY);};
  cv.onmousemove=function(e){if(!paintDrawing)return;var r=cv.getBoundingClientRect();var nx=e.clientX-r.left,ny=e.clientY-r.top;paintDo(paintLastX,paintLastY,nx,ny);paintLastX=nx;paintLastY=ny;};
  cv.onmouseup=cv.onmouseleave=function(){paintDrawing=false;};
  cv.ontouchstart=function(e){e.preventDefault();paintDrawing=true;var r=cv.getBoundingClientRect(),t=e.touches[0];paintLastX=t.clientX-r.left;paintLastY=t.clientY-r.top;paintDo(paintLastX,paintLastY,paintLastX,paintLastY);};
  cv.ontouchmove=function(e){e.preventDefault();if(!paintDrawing)return;var r=cv.getBoundingClientRect(),t=e.touches[0];var nx=t.clientX-r.left,ny=t.clientY-r.top;paintDo(paintLastX,paintLastY,nx,ny);paintLastX=nx;paintLastY=ny;};
  cv.ontouchend=function(){paintDrawing=false;};
}

function paintDo(x1,y1,x2,y2){
  if(!paintCtx||paintImageLoading) return;
  var sz=parseInt(document.getElementById('brushSize').value)||14;
  if(paintTool==='erase'){
    // Borracha: restaura fundo
...
function savePaint(){
  ...
  a.download='meu-dino-'+paintDino+'.png';
...
window.addEventListener('resize',function(){
  if(!document.getElementById('fightScreen').classList.contains('hidden')&&gs&&!gs.over){
    var cv=document.getElementById('gameCanvas'),fs=document.getElementById('fightScreen');
    cv.width=fs.clientWidth;
    var used=document.getElementById('fightHUD').offsetHeight+document.getElementById('spBars').offsetHeight+document.getElementById('touchCtrl').offsetHeight;
    cv.height=Math.max(fs.clientHeight-used,120);
  }
  if(!document.getElementById('paintScreen').classList.contains('hidden')&&paintCtx){
    var pcv=document.getElementById('paintCanvas');
    var tbH=document.getElementById('paintTopbar').offsetHeight;
    pcv.width=window.innerWidth;
    pcv.height=Math.max(window.innerHeight-tbH-10,280);
    redrawPaintBg();
  }
});
